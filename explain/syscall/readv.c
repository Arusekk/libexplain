...
void
explain_syscall_readv(int errnum, int argc, char **argv)
{
    int             fildes;
    struct iovec    *iov;
    int             iovcnt;
    ...
    if (0 == (argc & 1))
    {
        fprintf(stderr, "readv: requires an odd number of arguments, not %d\n",
            argc);
        exit(EXIT_FAILURE);
    }
    iovcnt = (argc - 1) / 2;
    ...
    explain_wrap_and_print(stdout, explain_errno_readv(errnum, fildes, iov,
        iovcnt));
}
