...
/**
  * The libexplain_listen_or_die function is
  * used to call the listen(2) system call.  On
  * failure an explanation will be printed to stderr,
  * obtained from libexplain_listen(3), and
  ...
  * @code
  * libexplain_listen_or_die(fildes, backlog);
  * @endcode
  *
  * @param fildes
  *     The fildes, exactly as to be passed to the listen(2) system call.
  * @param backlog
  *     The backlog, exactly as to be passed to the listen(2) system call.
  * @returns
  *     This function only returns on success.
  *     On failure, prints an explanation and exits,
  *     it does not return.
  */
void libexplain_listen_or_die(int fildes, int backlog);

/**
  * The libexplain_listen function is used to
  ...
  * {
  *     fprintf(stderr, "%s\n", libexplain_listen(fildes, backlog));
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  *
  * The above code example is available pre-packaged as
  * the #libexplain_listen_or_die function.
  *
  * @param fildes
  ...
  */
const char *libexplain_listen(int fildes, int backlog)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_errno_listen function is
  ...
  * {
  *     int err = errno;
  *     fprintf(stderr, "%s\n", libexplain_listen(err, fildes, backlog));
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  *
  ...
  *     many other functions in this library.
  */
const char *libexplain_errno_listen(int errnum, int fildes, int backlog)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_message_listen function is
  * used to obtain an explanation of an error returned by
  * the listen(2) system call.
  * The least the message will contain is the value of
  * strerror(errno), but usually it will do much better,
  ...
  * {
  *     char message[3000];
  *     libexplain_message_listen(message, sizeof(message), fildes, backlog);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  ...
  *     The original backlog, exactly as passed to the listen(2) system call.
  */
void libexplain_message_listen(char *message, int message_size, int fildes,
    int backlog);

/**
  * The libexplain_message_errno_listen
  ...
  *     int err = errno;
  *     char message[3000];
  *     libexplain_message_errno_listen(message, sizeof(message),
  *         err, fildes, backlog);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  ...
  *     The original backlog, exactly as passed to the listen(2) system call.
  */
void libexplain_message_errno_listen(char *message, int message_size,
