...
/**
  * The libexplain_execvp_or_die function is
  * used to call the execvp(3) system call.  On
  * failure an explanation will be printed to stderr,
  * obtained from libexplain_execvp(3), and
  ...
  * @code
  * libexplain_execvp_or_die(pathname, argv);
  * @endcode
  *
  * @param pathname
  ...
  *     it does not return.
  */
void libexplain_execvp_or_die(const char *pathname, char *const *argv);

/**
  * The libexplain_execvp function is used to
  * obtain an explanation of an error returned by the
  * execvp(3) system call.
  ...
  * fprintf(stderr, "%s\n", libexplain_execvp(pathname, argv));
  * exit(EXIT_FAILURE);
  * @endcode
  *
  * The above code example is available pre-packaged as
  * the #libexplain_execvp_or_die function.
  *
  * @param pathname
  ...
  */
const char *libexplain_execvp(const char *pathname, char *const *argv)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_errno_execvp function is
  ...
  * execvp(pathname, argv);
  * int err = errno;
  * fprintf(stderr, "%s\n", libexplain_execvp(err, pathname, argv));
  * exit(EXIT_FAILURE);
  * @endcode
  *
  * The above code example is available pre-packaged as
  ...
  *     many other functions in this library.
  */
const char *libexplain_errno_execvp(int errnum, const char *pathname,
    char *const *argv)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_message_execvp function is
  * used to obtain an explanation of an error returned by
  * the execvp(3) system call.
  * The least the message will contain is the value of
  * strerror(errno), but usually it will do much better,
  ...
  * execvp(pathname, argv);
  * char message[3000];
  * libexplain_message_execvp(message, sizeof(message), pathname, argv);
  * fprintf(stderr, "%s\n", message);
  * exit(EXIT_FAILURE);
  * @endcode
  *
  ...
  *     The original argv, exactly as passed to the execvp(3) system call.
  */
void libexplain_message_execvp(char *message, int message_size,
    const char *pathname, char *const *argv);

/**
  * The libexplain_message_errno_execvp
  ...
  * int err = errno;
  * char message[3000];
  * libexplain_message_errno_execvp(message, sizeof(message),
  *     err, pathname, argv);
  * fprintf(stderr, "%s\n", message);
  * exit(EXIT_FAILURE);
  * @endcode
  ...
  *     The original argv, exactly as passed to the execvp(3) system call.
  */
void libexplain_message_errno_execvp(char *message, int message_size,
