...
/**
  * The libexplain_socket_or_die function is
  * used to call the socket(2) system call.  On
  * failure an explanation will be printed to stderr,
  * obtained from libexplain_socket(3), and
...
/**
  ...
  * {
  *     int err = errno;
  *     fprintf(stderr, "%s\n", libexplain_socket(err, domain, type, protocol));
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  *
...
/**
  ...
  * {
  *     char message[3000];
  *     libexplain_message_socket(message, sizeof(message),
  ...
  *     The original protocol, exactly as passed to the socket(2) system call.
  */
void libexplain_message_socket(char *message, int message_size, int domain,
    int type, int protocol);

/**
  * The libexplain_message_errno_socket
  ...
  *     int err = errno;
  *     char message[3000];
  *     libexplain_message_errno_socket(message, sizeof(message), err,
  *         domain, type, protocol);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  ...
  *     The original protocol, exactly as passed to the socket(2) system call.
  */
void libexplain_message_errno_socket(char *message, int message_size,
