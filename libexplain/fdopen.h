...
/**
  * The libexplain_fdopen_or_die function is
  * used to call the fdopen(3) system call.  On
  * failure an explanation will be printed to stderr,
  * obtained from libexplain_fdopen(3), and
  ...
  * @code
  * FILE *fp = libexplain_fdopen_or_die(fildes, flags);
  * @endcode
  *
  * @param fildes
  *     The fildes, exactly as to be passed to the fdopen(3) system call.
  * @param flags
  *     The flags, exactly as to be passed to the fdopen(3) system call.
  * @returns
  *     This function only returns on success, see fdopen(3) for more
  *     information.  On failure, prints an explanation and exits, it
  *     does not return.
  */
FILE *libexplain_fdopen_or_die(int fildes, const char *flags);

/**
  * The libexplain_fdopen function is used to
  * obtain an explanation of an error returned by the
  * fdopen(3) system call.
  ...
  * if (!fp)
  * {
  *     fprintf(stderr, "%s\n", libexplain_fdopen(fildes, flags));
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  *
  * The above code example is available pre-packaged as
  * the #libexplain_fdopen_or_die function.
  *
  * @param fildes
  ...
  */
const char *libexplain_fdopen(int fildes, const char *flags)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_errno_fdopen function is
  ...
  * {
  *     int err = errno;
  *     fprintf(stderr, "%s\n", libexplain_fdopen(err, fildes, flags));
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  *
  ...
  *     many other functions in this library.
  */
const char *libexplain_errno_fdopen(int errnum, int fildes, const char *flags)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_message_fdopen function is
  * used to obtain an explanation of an error returned by
  * the fdopen(3) system call.
  * The least the message will contain is the value of
  * strerror(errno), but usually it will do much better,
  ...
  * {
  *     char message[3000];
  *     libexplain_message_fdopen(message, sizeof(message), fildes, flags);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  ...
  *     The original flags, exactly as passed to the fdopen(3) system call.
  */
void libexplain_message_fdopen(char *message, int message_size, int fildes,
    const char *flags);

/**
  * The libexplain_message_errno_fdopen
  ...
  *     int err = errno;
  *     char message[3000];
  *     libexplain_message_errno_fdopen(message, sizeof(message), err,
  *         fildes, flags);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  ...
  *     The original flags, exactly as passed to the fdopen(3) system call.
  */
void libexplain_message_errno_fdopen(char *message, int message_size,
