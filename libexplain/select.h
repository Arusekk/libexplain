...
/**
  * The libexplain_select_or_die function is
  * used to call the select(2) system call.  On
  * failure an explanation will be printed to stderr,
  * obtained from libexplain_select(3), and
  ...
  * @code
  * libexplain_select_or_die(nfds, readfds, writefds, exceptfds, timeout);
  * @endcode
  *
  * @param nfds
  ...
  *     it does not return.
  */
int libexplain_select_or_die(int nfds, fd_set *readfds, fd_set *writefds,
    fd_set *exceptfds, struct timeval *timeout);

/**
  * The libexplain_select function is used to
  ...
  */
const char *libexplain_select(int nfds, fd_set *readfds, fd_set *writefds,
    fd_set *exceptfds, struct timeval *timeout)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_errno_select function is
  ...
  *     many other functions in this library.
  */
const char *libexplain_errno_select(int errnum, int nfds, fd_set *readfds,
    fd_set *writefds, fd_set *exceptfds, struct timeval *timeout)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_message_select function is
  * used to obtain an explanation of an error returned by
  * the select(2) system call.
  * The least the message will contain is the value of
  * strerror(errno), but usually it will do much better,
  ...
  * {
  *     char message[3000];
  *     libexplain_message_select(message, sizeof(message), nfds,
  *         readfds, writefds, exceptfds, timeout);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  ...
  *     The original timeout, exactly as passed to the select(2) system call.
  */
void libexplain_message_select(char *message, int message_size, int nfds,
    fd_set *readfds, fd_set *writefds, fd_set *exceptfds,
    struct timeval *timeout);

/**
  * The libexplain_message_errno_select
  ...
  *     int err = errno;
  *     char message[3000];
  *     libexplain_message_errno_select(message, sizeof(message), err,
  ...
  *     The original timeout, exactly as passed to the select(2) system call.
  */
void libexplain_message_errno_select(char *message, int message_size,
