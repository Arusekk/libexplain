...
/**
  * The libexplain_getcwd_or_die function is
  * used to call the getcwd(2) system call.  On
  * failure an explanation will be printed to stderr,
  * obtained from libexplain_getcwd(3), and
  ...
  * @code
  * libexplain_getcwd_or_die(data, data_size);
  * @endcode
  *
  * @param data
  *     The data, exactly as to be passed to the getcwd(2) system call.
  * @param data_size
  *     The data_size, exactly as to be passed to the getcwd(2) system call.
  * @returns
  *     This function only returns on success, see getcwd(3) for more
  *     information..  On failure, prints an explanation and exits, it
  *     does not return.
  */
char *libexplain_getcwd_or_die(char *data, size_t data_size);

/**
  * The libexplain_getcwd function is used to
  ...
  * {
  *     fprintf(stderr, "%s\n", libexplain_getcwd(data, data_size));
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  *
  * The above code example is available pre-packaged as
  * the #libexplain_getcwd_or_die function.
  *
  * @param data
  ...
  */
const char *libexplain_getcwd(char *data, size_t data_size)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_errno_getcwd function is
  ...
  * {
  *     int err = errno;
  *     fprintf(stderr, "%s\n", libexplain_getcwd(err, data, data_size));
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  *
  ...
  *     many other functions in this library.
  */
const char *libexplain_errno_getcwd(int errnum, char *data, size_t data_size)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_message_getcwd function is
  * used to obtain an explanation of an error returned by
  * the getcwd(2) system call.
  * The least the message will contain is the value of
  * strerror(errno), but usually it will do much better,
  ...
  * {
  *     char message[3000];
  *     libexplain_message_getcwd(message, sizeof(message), data, data_size);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  ...
  * @param message_size
  *     The data_size in bytes of the location in which to
  ...
  *     The original data_size, exactly as passed to the getcwd(2) system call.
  */
void libexplain_message_getcwd(char *message, int message_size, char *data,
    size_t data_size);

/**
  * The libexplain_message_errno_getcwd
  ...
  *     int err = errno;
  *     char message[3000];
  *     libexplain_message_errno_getcwd(message, sizeof(message),
  *         err, data, data_size);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  ...
  *     The original data_size, exactly as passed to the getcwd(2) system call.
  */
void libexplain_message_errno_getcwd(char *message, int message_size,
