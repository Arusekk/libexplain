...
/**
  * The libexplain_dup2_or_die function is
  * used to call the dup2(2) system call.  On
  * failure an explanation will be printed to stderr,
  * obtained from libexplain_dup2(3), and
  ...
  *     it does not return.
  */
void libexplain_dup2_or_die(int oldfd, int newfd);

/**
  * The libexplain_dup2 function is used to
  ...
  */
const char *libexplain_dup2(int oldfd, int newfd)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_errno_dup2 function is
  ...
  * {
  *     int err = errno;
  *     fprintf(stderr, "%s\n", libexplain_dup2(err, oldfd, newfd));
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  *
  ...
  *     many other functions in this library.
  */
const char *libexplain_errno_dup2(int errnum, int oldfd, int newfd)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_message_dup2 function is
  * used to obtain an explanation of an error returned by
  * the dup2(2) system call.
  * The least the message will contain is the value of
  * strerror(errno), but usually it will do much better,
  ...
  * {
  *     char message[3000];
  *     libexplain_message_dup2(message, sizeof(message), oldfd, newfd);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  ...
  *     The original newfd, exactly as passed to the dup2(2) system call.
  */
void libexplain_message_dup2(char *message, int message_size, int oldfd,
    int newfd);

/**
  * The libexplain_message_errno_dup2
  ...
  *     int err = errno;
  *     char message[3000];
  *     libexplain_message_errno_dup2(message, sizeof(message), err,
  *         oldfd, newfd);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  ...
  *     The original newfd, exactly as passed to the dup2(2) system call.
  */
void libexplain_message_errno_dup2(char *message, int message_size, int errnum,
    int oldfd, int newfd);
