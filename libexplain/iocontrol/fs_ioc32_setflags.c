...
disambiguate(int fildes, int request, const void *data)
{
    ...
    (void)data;
    /* success = 0, failure = -1 */
    return (FS_IOC32_SETFLAGS == FS_IOC_SETFLAGS ? -1 : 0);
    /* We can't use #if because the definitions use sizeof. */
}


const explain_iocontrol_t explain_iocontrol_fs_ioc32_setflags =
{
    "FS_IOC32_SETFLAGS", /* name */
    FS_IOC32_SETFLAGS, /* value */
    disambiguate,
    0, /* print_name */
