...
/**
  * The libexplain_popen_or_die function is
  * used to call the popen(3) system call.  On
  * failure an explanation will be printed to stderr,
  * obtained from libexplain_popen(3), and
  ...
  * @code
  * FILE *fp = libexplain_popen_or_die(command, flags);
  * @endcode
  *
  * @param command
  ...
  *     does not return.
  */
FILE *libexplain_popen_or_die(const char *command, const char *flags);

/**
  * The libexplain_popen function is used to
  ...
  * if (!fp)
  * {
  *     fprintf(stderr, "%s\n", libexplain_popen(command, flags));
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  *
  * The above code example is available pre-packaged as
  * the #libexplain_popen_or_die function.
  *
  * @param command
  ...
  */
const char *libexplain_popen(const char *command, const char *flags)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_errno_popen function is
  ...
  * {
  *     int err = errno;
  *     fprintf(stderr, "%s\n", libexplain_popen(err, command, flags));
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  *
  ...
  *     many other functions in this library.
  */
const char *libexplain_errno_popen(int errnum, const char *command,
    const char *flags)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_message_popen function is
  * used to obtain an explanation of an error returned by
  * the popen(3) system call.
  * The least the message will contain is the value of
  * strerror(errno), but usually it will do much better,
  ...
  * {
  *     char message[3000];
  *     libexplain_message_popen(message, sizeof(message), command, flags);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  ...
  *     The original flags, exactly as passed to the popen(3) system call.
  */
void libexplain_message_popen(char *message, int message_size,
    const char *command, const char *flags);

/**
  * The libexplain_message_errno_popen
  ...
  *     int err = errno;
  *     char message[3000];
  *     libexplain_message_errno_popen(message, sizeof(message),
  *         err, command, flags);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  ...
  *     The original flags, exactly as passed to the popen(3) system call.
  */
void libexplain_message_errno_popen(char *message, int message_size, int errnum,
    const char *command, const char *flags);
