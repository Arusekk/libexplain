...
/**
  * The libexplain_fpathconf_or_die function is
  * used to call the fpathconf(3) system call.  On
  * failure an explanation will be printed to stderr,
  * obtained from libexplain_fpathconf(3), and
  ...
  * @code
  * long result = libexplain_fpathconf_or_die(fildes, name);
  * @endcode
  *
  * @param fildes
  *     The fildes, exactly as to be passed to the fpathconf(3) system call.
  * @param name
  *     The name, exactly as to be passed to the fpathconf(3) system call.
  * @returns
  *     This function only returns on success, see fpathconf(3) for more
  *     information.  On failure, prints an explanation and exits, it
  *     does not return.
  */
long libexplain_fpathconf_or_die(int fildes, int name);

/**
  * The libexplain_fpathconf function is used to
  * obtain an explanation of an error returned by the
  * fpathconf(3) system call.
  ...
  * {
  *     fprintf(stderr, "%s\n", libexplain_fpathconf(fildes, name));
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  *
  * The above code example is available pre-packaged as
  * the #libexplain_fpathconf_or_die function.
  *
  * @param fildes
  ...
  */
const char *libexplain_fpathconf(int fildes, int name)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_errno_fpathconf function is
  ...
  * {
  *     int err = errno;
  *     fprintf(stderr, "%s\n", libexplain_fpathconf(err, fildes, name));
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  *
  ...
  *     many other functions in this library.
  */
const char *libexplain_errno_fpathconf(int errnum, int fildes, int name)
                                                  LIBEXPLAIN_WARN_UNUSED_RESULT;

/**
  * The libexplain_message_fpathconf function is
  * used to obtain an explanation of an error returned by
  * the fpathconf(3) system call.
  * The least the message will contain is the value of
  * strerror(errno), but usually it will do much better,
  ...
  * {
  *     char message[3000];
  *     libexplain_message_fpathconf(message, sizeof(message), fildes, name);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  * @endcode
  ...
  *     The original name, exactly as passed to the fpathconf(3) system call.
  */
void libexplain_message_fpathconf(char *message, int message_size, int fildes,
    int name);

/**
  * The libexplain_message_errno_fpathconf
  ...
  *     int err = errno;
  *     char message[3000];
  *     libexplain_message_errno_fpathconf(message, sizeof(message),
  *         err, fildes, name);
  *     fprintf(stderr, "%s\n", message);
  *     exit(EXIT_FAILURE);
  * }
  ...
  *     The original name, exactly as passed to the fpathconf(3) system call.
  */
void libexplain_message_errno_fpathconf(char *message, int message_size,
